...
class RawImageApp(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.wlbtPanel.pack(side=tk.TOP, anchor=tk.W, fill=tk.BOTH, pady=10) #takes out the setting parameters
        self.ctrlPanel.pack(side=tk.TOP, anchor=tk.W, fill=tk.BOTH, pady=10)
        self.wlbt = Walabot() # iniitalizsing Walabot() as wlbt
    ...
    def loop(self):
        ...
        self.log.write(str(rawImage.tolist())+ "\n")
...
class WalabotPanel(tk.LabelFrame):
    ...
    def __init__(self, master):
        ...
        self.rRes = self.WalabotParameter(self, 'R     Res', 0.1, 10, 5.0)
        self.tMin = self.WalabotParameter(self, 'Theta Min', -90, 90, -30.0)
        self.tMax = self.WalabotParameter(self, 'Theta Max', -90, 90, 30.0)
        ...
        self.thld = self.WalabotParameter(self, 'Threshold', 0.1, 100, 75.0)
...
class CanvasPanel(tk.LabelFrame):
    ...
    def setGrid(self, sizeX, sizeY):
        ...
        #draws the individual cells in 
        self.cells = [[self.canvas.create_rectangle(recWidth*col, recHeight*row, recWidth*(col+1), recHeight*(row+1), width=1) for col in range(sizeY)] for row in range(sizeX)]
        # range(sizeY) controls the size of x-axis, range(sizeX) controls size of y-axis
        # rectangles are made by strating at top left point and extending down to bottom right point
    ...
    def update(self, rawImage, lenOfPhi, lenOfR):
        ...
        # Based on a certain threshold value, the cells are made to have a value of 0, which
        # corresponds to the darkest parts of the heatmap. In other words, if the recvied signal
        # at a certain location in the rawimage matrix is less than '70' (threshld in this case),
        # than all values <= 70 are chagned to 0  to reduce the noise
        for i in range(lenOfR):
            for j in range(lenOfPhi):
...
class Walabot:
    ...
    def triggerAndGetSensorTargets(self):
